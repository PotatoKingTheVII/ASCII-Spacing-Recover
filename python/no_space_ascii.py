...
def space_brute(ct, mode):
    ...
    if(mode == 2 or mode == 3):
        ...
        if(mode == 2):  #High bytes
            start = 7
            end = 0
            step = -1

        else:   #Low bytes (mode = 3)
            start = 1
            end = 8
            step = 1
    ...
    if(mode == 0):
        ...
        base = 2
    if(mode > 3):   #I.e. we worked backwards so reverse the array
...
#Modes: 0=Dec, 1=Oct, 2=Bin-forward-high, 3=Bin-forward-low, 4=Bin-backwards-high, 5=Bin-backwards-low
